...
export default function Experience({ experienceRef }) {
    ...
    return (
        <div className="mt-20" ref={experienceRef}>
            ...
            <div className="grid grid-cols-4 gap-y-10 place-content-center mt-10" onClick={() => setFocusedElement(null)}>
                {logoData?.map((logo, idx) =>
                    <div className="flex flex-col justify-center items-center relative">
                        <ImageKitWrapper className="rounded-2xl cursor-pointer" path={logo?.technology_image} width={100} height={100} />
                        <p className="text-hero_text-dark mt-5 cursor-pointer" onMouseOver={() => setFocusedElement(idx)} >{logo?.technology_name}</p>
